"use client";
import { AlignJustify, BookOpen, BookPlus, GraduationCap, MenuSquare, PieChart, School, School2, Users, X } from "lucide-react";
...
export default function Aside() {
	...
	return (
		<>
			...
			<aside className={` ${menu ? "flex h-full w-full bg-modal" : "hidden md:flex md:w-1/6 md:h-full bg-principal "} fixed top-0 left-0 flex-col gap-8 close`}>
					...
					<div className="w-full h-auto flex flex-col items-center justify-center gap-4">
						...
						<section className={`w-full h-auto ${menu ? "block" : "hidden sm:block"}`}>
							<ul className="flex flex-col flex-wrap gap-4">
								<Link onClick={() => setMenu(false)} href="/dashboard" className="w-full flex flex-row justify-center items-center py-2 hover:bg-[#458ACE]">
								...
								<Link onClick={() => setMenu(false)} href="/controle-aulas-eventuais" className="w-full flex flex-row justify-center items-center py-2 hover:bg-[#458ACE]">
									<li className="w-10/12 flex flex-row items-center gap-3 text-white">
								...
								<div className="w-full h-full flex flex-col items-center justify-center gap-4 cursor-pointer" onClick={() => setShowDropdownRegister(!showDropdownRegister)}>
									...
									{showDropdownRegister && (
										<ul className="w-full flex flex-col gap-4">
											<Link onClick={() => setMenu(false)} href="/cadastro-escola" className="w-full flex flex-row justify-center items-center py-2 hover:bg-[#458ACE]">
											...
											<Link onClick={() => setMenu(false)} href="/cadastro-professor" className="w-full flex flex-row justify-center items-center py-2 hover:bg-[#458ACE]">
												<li className="w-8/12 flex flex-row items-center gap-3 text-white">
								...
								<div className="w-full h-full flex flex-col gap-4 cursor-pointer" onClick={() => setShowDropdownReport(!showDropDownReport)}>
									...
									{showDropDownReport && (
										<ul className="w-full flex flex-col gap-4">
											<Link onClick={() => setMenu(false)} href="/boletim-substituicao" className="w-full flex flex-row justify-center items-center py-2 hover:bg-[#458ACE]">
												<li className="w-8/12 flex flex-row items-center gap-3 text-white">
													<School size={22} />
											...
											<Link onClick={() => setMenu(false)} href="/boletim-controle-aulas-eventuais" className="w-full flex flex-row justify-center items-center py-2 hover:bg-[#458ACE]">
												<li className="w-8/12 flex flex-row items-center gap-3 text-white">
													<GraduationCap size={22} />
						...
						<footer className={"w-10/12 flex flex-col gap-2 items-center justify-start absolute bottom-0"}>
							<span className="text-base text-white opacity-40 cursor-default">Alerrando © 2023 Obrigado por acessar</span>
						</footer>
					</div>
				</div>
			</aside>
