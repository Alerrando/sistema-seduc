...
export default function ImprimirBoletimControleAulasEventuais() {
  const { filterInfosTeacher, allFilterInfosTeacher } = useSelector(
      ...
      <main className="w-full h-screen">
        <section className="w-full-h-auto flex flex-col items-center justify-center gap-8 px-6 py-8 md:px-12 md:py-8">
              ...
              <div className="w-full flex items-center justify-between">
                <div className="w-auto flex flex-row items-center gap-1 md:gap-2">
                  ...
                  <span className="font-bold text-sm md:text-lg">
                    {filterInfosTeacher?.office.name}
                  </span>
                </div>
                ...
                <div className="w-auto flex flex-row items-center gap-1 md:gap-2">
                  ...
                  <span className="font-bold text-sm md:text-lg">
                    {filterInfosTeacher.thirst.name}
                  </span>
                </div>
          ...
          <table className="min-w-full border text-center text-sm font-light border-neutral-500">
            ...
            <tbody className="">
              {allFilterInfosTeacher.map(
                (info: TeacherDTOInfos, index: Key) => (
                  <tr
                    className="border-b border-neutral-500"
                    key={`imprimir-professor-tbody-${index}`}
                  >
