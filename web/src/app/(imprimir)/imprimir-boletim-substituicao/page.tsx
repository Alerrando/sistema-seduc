...
export default function ImprimirBoletimSubstituicao() {
  const { filterInfosSchool, allFilterInfosSchool, filterStartEndDate } =
    useSelector((root: RootState) => root.SliceFilter);
  const [usersMandatoryBulletin, setUsersMandatoryBulletin] = useState<
    UserInfos[]
  >([]);
  const [schoolBulletinUser, setSchoolBulletinUser] =
    useState<UserInfos>(DefaultUserInfos);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const auxSchoolBulletinUser = await getUserByIdSchool(
          ...
          <div className="w-full h-auto flex items-center justify-center">
            ...
            >
              {`Boletim de Substituição - Professores Eventuais - Mês ${
                ...
                isValid(new Date(filterStartEndDate.dataInicial))
                  ? format(
                      new Date(filterStartEndDate.dataInicial),
                      "dd/MM/yyyy",
                      {
                        locale: ptBR,
                      },
                ...
                isValid(new Date(filterStartEndDate.dataFinal))
                  ? format(
                      new Date(filterStartEndDate.dataFinal),
                      "dd/MM/yyyy",
                      {
                        locale: ptBR,
                      },
            ...
            <table className="min-w-full border text-center text-sm font-light border-neutral-500">
              <thead className="border-b font-medium border-neutral-500">
                <tr>
                  <th
                    scope="col"
                    className="text-start border-r pl-4 pr-2 py-2 border-neutral-500"
                  >
                    NOME
                  </th>
                  <th
                                  ...
                                  >
                                    <span>
                                      {isValid(new Date(dateWork[0]))
                                        ? format(new Date(dateWork[0]), "dd/MM")
                                        : ""}
                                    </span>
                                    <span>-</span>
                  ...
                  {user.id !== schoolBulletinUser.id ? (
                    <div className="w-1/4 h-auto flex flex-col items-center justify-center before:block before:w-full before:border-t-[1px] before:border-t-black">
                      ...
                      <span className="text-sm">{`${
                        user.office?.name === undefined ? "" : user.office?.name
                      }`}</span>
                    </div>
                  ) : null}
                </>
