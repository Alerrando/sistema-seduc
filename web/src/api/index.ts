...
export async function import axios from "axios";
import { DefinitionPeriodsInfos, LessonsInfos, OfficeInfos, SchoolDTOInfos, SchoolInfos, SchoolValuesDefault, TeacherDTOInfos, TeacherInfos, TeacherValuesDefault } from "../../slice";
import { UserInfos } from "../../slice/LoginSlide";
...
export async function getUserByMandatoryBulletin(){
  let message = await axios.get(`${urlUser}/bulletin`, {
    headers: { 'Authorization': `${localStorage.getItem("token")}` }
  })
  .then((res) => res.data)
  .catch((err) => err)
...
export async function getUserByIdSchool(schoolId: string){
  let message = await axios.get(`${urlUser}/school/${schoolId}`, {
    headers: { "Authorization": `${localStorage.getItem("token")}` }
  })
  .then((res) => res.data)
  .catch((err) => err)
